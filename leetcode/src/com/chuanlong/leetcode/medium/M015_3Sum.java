    	...
    	if(nums != null){
    		...
    		for(int i=0; i<nums.length; i++){
    			for(int j=i+1; j<nums.length; j++){
    				...
    				int index = splitSearch(nums, val, j+1, nums.length-1);
    				
    				if(index != -1){
    					List<Integer> item = new ArrayList<Integer>(Arrays.asList(nums[i], nums[j], nums[index]));
    ...
    private boolean isContain(List<List<Integer>> list, List<Integer> item){
    	for(int i=0; i<list.size(); i++){
    		...
    		for(int j=0; j<item.size(); j++){
    			if(list.get(i).get(j) != item.get(j)){
    				isSame = false;
    				break;
    			}
    		}
